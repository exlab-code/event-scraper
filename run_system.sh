...
case "$1" in
    ...
    all)
        ...
        echo "Use './run_system.sh stop' to stop all components."
    ...
    setup-cron)
        ...
        (crontab -l 2>/dev/null; echo "0 1 * * * cd $(pwd) && $(pwd)/run_system.sh scrape >> $(pwd)/logs/scraper-cron.log 2>&1") | crontab -
        (crontab -l 2>/dev/null; echo "0 2 * * * cd $(pwd) && $(pwd)/run_system.sh analyze >> $(pwd)/logs/analysis-cron.log 2>&1") | crontab -
        (crontab -l 2>/dev/null; echo "0 * * * * pgrep -f \"python.*calendar_sync.py\" || cd $(pwd) && $(pwd)/run_system.sh sync >> $(pwd)/logs/sync-cron.log 2>&1") | crontab -
        echo "Cron jobs set up. Use 'crontab -l' to view them."
        ;;
    remove-cron)
        echo "Removing cron jobs..."
        crontab -l | grep -v "Event Scraper System\|run_system.sh" | crontab -
